...
class GraphEncoder(Encoder):
    ...
    def forward(self, embed_src: Tensor, src_length: Tensor, mask: Tensor,
        ...
        # apply dropout to the emmbeding input
        embeddings = self.emb_dropout(embed_src)
        batch_index_dict,batch_new_orgs,batch_new_trgs = self.bpe_changes(batch)
        #embed_edges = self.edge_embeddings(batch.edge)
        #embed_edges = self.emb_dropout(embed_edges)
        #embeddings= torch.cat((embed_src,embed_edges),dim=1)
        #pdb.set_trace()
        #try:
        data=self.reorder_edges_words(embeddings,batch,\
                   batch_index_dict,batch_new_orgs,batch_new_trgs)
            #pdb.set_trace()
        x, edge_index = data.x, data.edge_index
            #pdb.set_trace()
        x = self.ggnn1(x, edge_index.cuda())
    ...
    def bpe_changes(self,batch):
        ...
        for i,sentence in enumerate(batch.src):
            ...
            for j,word in enumerate(sentence):
                if past_word==True:
                  sentence_dict[index]=j
                if not("@@" in self.source_vocab.itos[word]):                  
                    index+=1
                    past_word=True
                else:
                  past_word=False
                  sentence_new_orgs.append(j)
                  sentence_new_trgs.append(j+1)
                sent.append(self.source_vocab.itos[word])
            batch_index_dict.append(sentence_dict)
            batch_new_orgs.append(sentence_new_orgs)
            batch_new_trgs.append(sentence_new_trgs)
            sentences.append(sent)
        #pdb.set_trace()

        return batch_index_dict,batch_new_orgs,batch_new_trgs
    
    def reorder_edges_words(self,embed,batch,batch_index_dict,batch_new_orgs,batch_new_trgs):
        """
        Input: batch with edge_org,edge_trg, and x
        return Batch of pytorch geometric
        """
            ...
            for j,edge_org in enumerate(edge_orgs): 
                ...
                if not(int(org)==0 and int(trg)==0):
                    ...
                    if int(trg)>curr_lenght:
                        ...
                        trgs.append(int(trg))
                    """
                    orgs.append(batch_index_dict[i][int(org)])
                    trgs.append(batch_index_dict[i][int(trg)])
            #pdb.set_trace()
            orgs=orgs+batch_new_orgs[i]
            trgs=trgs+batch_new_trgs[i]
            #pdb.set_trace()
            data_list.append(Data(embed[i],torch.tensor([orgs,trgs],dtype=torch.long)))
        #pdb.set_trace()
        return Batch.from_data_list(data_list)
